// 合并 40 个 extract_th_partXX.json → messages/th.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'th.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_th_part${num}.json`);
...
// 应用补译补丁：messages/work/th_fix.json（扁平 key.path -> 值，覆盖残留英文）
const FIX = path.join(__dirname, '..', 'th_fix.json');
...
const thPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const thArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !thPaths.includes(x));
const extra = thPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !thArrays.includes(x));
const extraArr = thArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, th 叶子: ${thPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, th 数组键: ${thArrays.length}`);
...
console.log(`[TODO] 占位符叶子: th=${todos.length} (en=${enTodo.length})`);

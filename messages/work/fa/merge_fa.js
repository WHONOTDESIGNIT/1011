// 合并 40 个 extract_fa_partXX.json → messages/fa.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'fa.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_fa_part${num}.json`);
...
// 应用补译补丁：messages/work/fa_fix.json（扁平 key.path -> 值，覆盖残留英文）
const FIX = path.join(__dirname, '..', 'fa_fix.json');
...
const faPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const faArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !faPaths.includes(x));
const extra = faPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !faArrays.includes(x));
const extraArr = faArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, fa 叶子: ${faPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, fa 数组键: ${faArrays.length}`);
...
console.log(`[TODO] 占位符叶子: fa=${todos.length} (en=${enTodo.length})`);

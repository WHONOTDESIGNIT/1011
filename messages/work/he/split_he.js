// 拆 en.json 为 40 个小单元（按叶子路径深拆，保留顶层命名空间），供希伯来语翻译派发
const fs = require('fs');
...
function buildNested(paths) {
  ...
  for (const p of paths) {
    ...
    // 值引用 en 原文（subagent 以此为源翻译成希伯来语）
    cur[last] = getVal(en, p);
  }
  return root;
}
...
chunks.forEach((chunk, i) => {
  ...
  console.log(`part${num}: ${chunk.length} 叶子, 顶层键: ${Object.keys(block).join(',')}`);
});

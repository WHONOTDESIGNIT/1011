// 合并 40 个 extract_ko_partXX.json → messages/ko.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'ko.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_ko_part${num}.json`);
...
const koPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const koArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !koPaths.includes(x));
const extra = koPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !koArrays.includes(x));
const extraArr = koArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, ko 叶子: ${koPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, ko 数组键: ${koArrays.length}`);
...
console.log(`[TODO] 占位符叶子: ko=${todos.length} (en=${enTodo.length})`);

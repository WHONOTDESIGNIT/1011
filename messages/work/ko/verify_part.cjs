// 逐轮质量校验：对比指定分片的 en 源与 ko 目标（叶子路径 + 数组结构 + TODO 对齐）
...
for (const num of nums) {
  ...
  const koFile = path.join(DIR, `extract_ko_part${num}.json`);
  if (!fs.existsSync(koFile)) { console.log(`part${num}: ❌ 缺少目标文件`); allOk = false; continue; }
  let en, ko;
  try {
    en = JSON.parse(fs.readFileSync(enFile, 'utf8'));
    ko = JSON.parse(fs.readFileSync(koFile, 'utf8'));
  } catch (e) { console.log(`part${num}: ❌ JSON 解析失败: ${e.message}`); allOk = false; continue; }
  const enL = collectLeaves(en, '', []).sort();
  const koL = collectLeaves(ko, '', []).sort();
  const enA = collectArrays(en, '', []).sort();
  const koA = collectArrays(ko, '', []).sort();
  const missing = enL.filter((x) => !koL.includes(x));
  const extra = koL.filter((x) => !enL.includes(x));
  const missingArr = enA.filter((x) => !koA.includes(x));
  const extraArr = koA.filter((x) => !enA.includes(x));
  ...
  console.log(`part${num}: 叶子 ${enL.length}/${koL.length} 数组 ${enA.length}/${koA.length} ${ok ? '✅' : '❌'}`);

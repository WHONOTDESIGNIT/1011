// 合并 40 个 extract_ro_partXX.json → messages/ro.json，并校验与 en.json 键路径完全一致
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'ro.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_ro_part${num}.json`);
...
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致，
// 与既有语言包（nl/es/fr/he 等）的约定一致，避免子代理重建不存在的源文案
...
const roPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const roArrays = collectArrays(merged, '', []).sort();
...
console.log(`en 数组键: ${enArrays.length}, ro 数组键: ${roArrays.length}`);
...
for (const p of roPaths) {
  ...
  if (typeof cur === 'string' && /\b(the|your|our|with|for|from|this|that|what|why|how|are|were|will|and)\b/.test(cur) && cur.length > 12) {
    engLeaves++;

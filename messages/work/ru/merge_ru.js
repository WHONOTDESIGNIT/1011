// 合并 40 个 extract_ru_partXX.json → messages/ru.json，并校验与 en.json 键路径完全一致
// 附：俄语标点规范化——移除 ? ! : ; 前的多余空格（俄语规范：标点前不加空格）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'ru.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_ru_part${num}.json`);
...
const ruPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const ruArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !ruPaths.includes(x));
const extra = ruPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !ruArrays.includes(x));
const extraArr = ruArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, ru 叶子: ${ruPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, ru 数组键: ${ruArrays.length}`);

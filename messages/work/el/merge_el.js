// 合并 40 个 extract_el_partXX.json → messages/el.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'el.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_el_part${num}.json`);
...
// 应用补译补丁：messages/work/el_fix.json（扁平 key.path -> 值，覆盖残留英文）
const FIX = path.join(__dirname, '..', 'el_fix.json');
...
const elPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const elArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !elPaths.includes(x));
const extra = elPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !elArrays.includes(x));
const extraArr = elArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, el 叶子: ${elPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, el 数组键: ${elArrays.length}`);
...
console.log(`[TODO] 占位符叶子: el=${todos.length} (en=${enTodo.length})`);

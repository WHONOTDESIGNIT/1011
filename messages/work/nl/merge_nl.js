// 合并 40 个 extract_nl_partXX.json → messages/nl.json，并校验与 en.json 键路径完全一致
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'nl.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_nl_part${num}.json`);
...
const nlPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const nlArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !nlPaths.includes(x));
const extra = nlPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !nlArrays.includes(x));
const extraArr = nlArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, nl 叶子: ${nlPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, nl 数组键: ${nlArrays.length}`);

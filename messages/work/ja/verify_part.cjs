// 逐轮质量校验：对比指定分片的 en 源与 ja 目标（叶子路径 + 数组结构 + TODO 对齐）
...
for (const num of nums) {
  ...
  const jaFile = path.join(DIR, `extract_ja_part${num}.json`);
  if (!fs.existsSync(jaFile)) { console.log(`part${num}: ❌ 缺少目标文件`); allOk = false; continue; }
  let en, ja;
  try {
    en = JSON.parse(fs.readFileSync(enFile, 'utf8'));
    ja = JSON.parse(fs.readFileSync(jaFile, 'utf8'));
  } catch (e) { console.log(`part${num}: ❌ JSON 解析失败: ${e.message}`); allOk = false; continue; }
  const enL = collectLeaves(en, '', []).sort();
  const jaL = collectLeaves(ja, '', []).sort();
  const enA = collectArrays(en, '', []).sort();
  const jaA = collectArrays(ja, '', []).sort();
  const missing = enL.filter((x) => !jaL.includes(x));
  const extra = jaL.filter((x) => !enL.includes(x));
  const missingArr = enA.filter((x) => !jaA.includes(x));
  const extraArr = jaA.filter((x) => !enA.includes(x));
  ...
  console.log(`part${num}: 叶子 ${enL.length}/${jaL.length} 数组 ${enA.length}/${jaA.length} ${ok ? '✅' : '❌'}`);

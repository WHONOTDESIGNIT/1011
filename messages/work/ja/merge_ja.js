// 合并 40 个 extract_ja_partXX.json → messages/ja.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'ja.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_ja_part${num}.json`);
...
const jaPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const jaArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !jaPaths.includes(x));
const extra = jaPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !jaArrays.includes(x));
const extraArr = jaArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, ja 叶子: ${jaPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, ja 数组键: ${jaArrays.length}`);
...
console.log(`[TODO] 占位符叶子: ja=${todos.length} (en=${enTodo.length})`);

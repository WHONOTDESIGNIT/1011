// 合并 40 个 extract_cs_partXX.json → messages/cs.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'cs.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_cs_part${num}.json`);
...
// 应用补译补丁：messages/work/cs_fix.json（扁平 key.path -> 值，覆盖残留英文）
const FIX = path.join(__dirname, '..', 'cs_fix.json');
...
const csPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const csArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !csPaths.includes(x));
const extra = csPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !csArrays.includes(x));
const extraArr = csArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, cs 叶子: ${csPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, cs 数组键: ${csArrays.length}`);
...
console.log(`[TODO] 占位符叶子: cs=${todos.length} (en=${enTodo.length})`);

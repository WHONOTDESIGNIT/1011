// 合并 40 个 extract_id_partXX.json → messages/id.json，并校验与 en.json 键路径完全一致
// 归一化：英文源为 [TODO] 的遗留键（站点未使用）保持与 en.json 完全一致（与既有语言包约定一致）
const fs = require('fs');
const path = require('path');

const EN = path.join(__dirname, '..', '..', 'en.json');
const OUT = path.join(__dirname, '..', '..', 'id.json');
...
for (let i = 1; i <= PARTS; i++) {
  ...
  const file = path.join(SPLIT_DIR, `extract_id_part${num}.json`);
...
const idPaths = collectPaths(merged, '', []).sort();
const enArrays = collectArrays(en, '', []).sort();
const idArrays = collectArrays(merged, '', []).sort();

const missing = enPaths.filter((x) => !idPaths.includes(x));
const extra = idPaths.filter((x) => !enPaths.includes(x));
const missingArr = enArrays.filter((x) => !idArrays.includes(x));
const extraArr = idArrays.filter((x) => !enArrays.includes(x));

console.log(`en 叶子: ${enPaths.length}, id 叶子: ${idPaths.length}`);
...
console.log(`en 数组键: ${enArrays.length}, id 数组键: ${idArrays.length}`);
...
console.log(`[TODO] 占位符叶子: id=${todos.length} (en=${enTodo.length})`);
